...
ARDrawingContext::ARDrawingContext(std::string windowName ,
	cv::Size frameSize , const CameraCalibration& c):m_isTextureInitialized(false) , m_calibration(c), m_windowName(windowName)
{
	// 创建一个用于显示3D图像的窗体，指定此窗体支持OpenGL
	cv::namedWindow(windowName , cv::WINDOW_OPENGL);

	// 将窗体的尺寸设置为与从摄像头读取的帧的尺寸相同
	cv::resizeWindow(windowName , frameSize.width , frameSize.height);

	// 初始化使用OpenGL的3D功能时需要注册的回调函数
	cv::setOpenGlContext(windowName);		// 社会OpenGL需要支持的窗体
...
void ARDrawingContext::updateWindow(const cv::Mat& frame)
{
...
void ARDrawingContext::drawCameraFrame()
{
	...
	// 指定像素数据在内存中的存放方式，由于一个像素所使用的字节数通常与内存的对其方式不同，因此，需要显式地设置对其方式
	glPixelStorei(GL_PACK_ALIGNMENT , 1);
	glBindTexture(GL_TEXTURE_2D , m_backgroundTextureId);

	// 更新纹理数据
	if(m_backgroundImage.channels() ==3)
		glTextImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, m_backgroundImage.data);
	else if(m_backgroundImage.channels() == 4)
		glTextImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, m_backgroundImage.data);
	else if(m_backgroundImage.channels() ==1)
		glTextImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, m_backgroundImage.data);
...
// 根据标定后的相机，以及屏幕的尺寸来计算投影转换（映射）矩阵
void ARDrawingContext::buildProjectionMatrix(CameraCalibration& calibration,
	int srceen_width , int srceen_height , Matrix44& projectionMatrix)
{
